...
#pragma config(Motor,  port3,           FRM,           tmotorVex393_MC29, openLoop, reversed, driveRight)
#pragma config(Motor,  port4,           BLM,           tmotorVex393_MC29, openLoop, driveLeft)
#pragma config(Motor,  port5,           BRM,           tmotorVex393_MC29, openLoop, reversed, driveRight)
...
task main()
{
	...
	while(true)
	{
		...
		if(B8L > B8LprevVal)//If B8L pressed controlMode--
		{
			...
			controlMode = ((controlMode-1)% NUM_TOTAL_OP_MODES)+1;//Add one to mode, wrap around
			startTask(flashControlMode);
			if(controlMode == 4&&prevMode != controlMode)
			{
				clearTimer(T1);//Clear and start timer T1
				prevTime = time1[T1];
			}
